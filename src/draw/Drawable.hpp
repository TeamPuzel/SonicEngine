...
//
// This is a C++23 port of the design. Because it relies on extending existing types rather
...
// For an example look into the extend(in direction:by amount:) message which changes the bounds of a view.
//
...
/// Performs forwarding adapter composition. Based on std::ranges.
...
auto operator==(draw::SizedDrawable auto const& lhs, draw::SizedDrawable auto const& rhs) -> bool {
...
namespace draw {
    ...
    adapt::Clear clear(Color color = color::CLEAR) {
        return adapt::Clear { color };
    }

    // FIXME: This is supposed to use blend modes.
    adapt::Pixel pixel(i32 x, i32 y, Color color) {
        return adapt::Pixel { x, y, color };
    }
}
