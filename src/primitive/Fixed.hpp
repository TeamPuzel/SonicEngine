...
class [[clang::trivial_abi]] fixed final { // NOLINT(readability-identifier-naming)
    ...
    constexpr explicit fixed(u32 raw) : raw(raw) {}

    public:
    [[clang::always_inline]]
    constexpr explicit(false) fixed() : raw(0) {}
    ...
    [[clang::always_inline]]
    constexpr explicit(false) fixed(i32 whole, u8 fraction = 0)
        : raw(u32((whole << 8) + ((i32(fraction) ^ (whole >> 31)) - (whole >> 31)))) {}

    [[clang::always_inline]]
    constexpr explicit(false) operator i32() const {
...
[[clang::always_inline]]
constexpr auto operator-(fixed self) -> fixed {
